...
Write-Host 'CHECK background working set: start'
...
if (-not $report.within_idle_working_set_limit) {
    throw (
        'Idle background working set exceeded 2 MiB: ' +
        "$($report.final_working_set_bytes) bytes."
    )
}

Write-Host 'CHECK background working set: passed'
