...
const USER_AGENT: &str = "ntfy-windows-client/0.1";
...
fn validate_config(config: &ClientConfig) -> Result<(), Error> {
    ...
    if !valid_topic(&config.topic) {
        return Err(Error("topic must be 1-64 letters, digits, '-' or '_'".into()));
    }
...
fn run_subscription(
    ...
    while !stop.load(Ordering::Acquire) {
        ...
        let mut handle_message = |message| {
                ...
                if config.notify {
                    let toast_title = if message.title.is_empty() {
                        format!("ntfy · {}", message.topic)
                    } else {
                        message.title.clone()
                    };
                    let _ = crate::toast::show(&toast_title, &message.body);
                }
    ...
    F: FnMut(Message),
{
    ...
    let object = format!("{}/{}/json?since={since}", server.base_path, config.topic);
    ...
    unsafe {
        WinHttpSetTimeouts(session.0, 5_000, 10_000, 10_000, 90_000);
    }
    ...
    let request_raw = unsafe {
        WinHttpOpenRequest(
            ...
            ptr::null(),
            ptr::null_mut(),
            if server.secure { WINHTTP_FLAG_SECURE as u32 } else { 0 },
        )
    };
    ...
    on_message_status_connected(active_request, request.raw)?;
    on_connected();
    ...
    loop {
        ...
        let read_len = available.min(64 * 1024) as usize;
        let start = pending.len();
        if start.saturating_add(read_len) > MAX_LINE_BYTES {
            return Err(Error("incoming ntfy event exceeded 1 MiB".into()));
        }
        pending.resize(start + read_len, 0);
        let mut read = 0_u32;
        winhttp_bool(
            unsafe {
                WinHttpReadData(
...
fn publish_blocking(config: &ClientConfig, title: &str, body: &str) -> Result<(), Error> {
    ...
    let server = ParsedServer::parse(&config.server_url)?;
    let object = format!("{}/{}", server.base_path, config.topic);
    let session = Handle::new(unsafe {
        WinHttpOpen(
            wide(USER_AGENT).as_ptr(),
            WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY as u32,
    ...
    unsafe {
        WinHttpSetTimeouts(session.0, 5_000, 10_000, 10_000, 30_000);
    }
    let host = wide(&server.host);
    let connect = Handle::new(unsafe { WinHttpConnect(session.0, host.as_ptr(), server.port, 0) })?;
    ...
    let title = sanitize_header(title, 256);
    if !title.is_empty() {
...
impl<'a> RequestHandle<'a> {
    fn new(raw: HINTERNET, registry: &'a AtomicPtr<c_void>) -> Result<Self, Error> {
        if raw.is_null() {
            let code = unsafe { GetLastError() };
            return Err(Error(format!("WinHTTP handle creation failed (Windows error {code})")));
        }
...
impl Drop for RequestHandle<'_> {
    fn drop(&mut self) {
        if self
            .registry
            .compare_exchange(self.raw, ptr::null_mut(), Ordering::AcqRel, Ordering::Acquire)
            .is_ok()
        {
            unsafe {
...
impl Handle {
    fn new(raw: HINTERNET) -> Result<Self, Error> {
        if raw.is_null() {
            let code = unsafe { GetLastError() };
            Err(Error(format!("WinHTTP handle creation failed (Windows error {code})")))
        } else {
            Ok(Self(raw))
        }
    }
...
impl ParsedServer {
    fn parse(input: &str) -> Result<Self, Error> {
        ...
        Ok(Self { secure, host, port, base_path })
    }
}

fn parse_authority(authority: &str, default_port: u16) -> Result<(String, u16), Error> {
    if let Some(rest) = authority.strip_prefix('[') {
        let end = rest.find(']').ok_or_else(|| Error("invalid IPv6 address".into()))?;
        let host = &rest[..end];
    ...
    if let Some((host, port)) = authority.rsplit_once(':') {
        ...
        let port = port.parse::<u16>().map_err(|_| Error("invalid server port".into()))?;
        if host.is_empty() {
...
mod tests {
    ...
    fn parses_ipv6_authority() {
        assert_eq!(parse_authority("[::1]:8080", 80).unwrap(), ("::1".into(), 8080));
    }
}
